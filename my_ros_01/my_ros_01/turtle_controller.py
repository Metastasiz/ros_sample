#!/usr/bin/env python3
import math
import rclpy
from rclpy.node import Node
from functools import partial

from turtlesim.msg import Pose
from geometry_msgs.msg import Twist

from my_ros_interface.msg import Turtle
from my_ros_interface.msg import TurtleArray
from my_ros_interface.srv import CatchTurtle

class TurtleController(Node): #modify name
	def __init__(self):
		super().__init__("turtle_controller") #modify name
		#contants
		self.parm_catch_closest_first__ = "catch_closest_turtle_first"

		#parameters
		self.declare_parameter(self.parm_catch_closest_first__,True)

		self.catch_closest_turtle_first_ = self.get_parameter(
			self.parm_catch_closest_first__).value
		self.turtle_to_catch_ = None
		self.pose_ = None
		
		self.publisher_cmd_vel_ = self.create_publisher(
			Twist, "turtle1/cmd_vel", 10
		)
		self.subscriber_pose_ = self.create_subscription(
			Pose, "turtle1/pose", self.callback_turtle_pose, 10
		)
		self.subscriber_turtle_list_ = self.create_subscription(
			TurtleArray, "turtle_list", self.callback_turtle_list, 10
		)
		self.timer_control_loop_ = self.create_timer(
			0.01, self.control_loop
		)
	
	def callback_turtle_pose(self, msg):
		self.pose_ = msg

	def callback_turtle_list(self, msg):
		if len(msg.turtles) > 0:
			if self.catch_closest_turtle_first_:
				dist_min = None
				turtle_min = None
				for turtle in msg.turtles:
					dist_x = turtle.x - self.pose_.x
					dist_y = turtle.y - self.pose_.y
					dist_diff = math.sqrt(dist_x * dist_x + dist_y * dist_y)
					if 	turtle_min == None or\
						dist_diff < dist_min:
						turtle_min = turtle
						dist_min = dist_diff
				self.turtle_to_catch_ = turtle_min
			else:
				self.turtle_to_catch_ = msg.turtles[0]

	def control_loop(self):
		#contants
		dist_stop = 0.5
		kp_linear = 2
		kp_angular = 6
		
		if 	self.pose_ == None or\
			self.turtle_to_catch_ == None:
			return
		
		dist_x = self.turtle_to_catch_.x - self.pose_.x
		dist_y = self.turtle_to_catch_.y - self.pose_.y
		dist_diff = math.sqrt(dist_x * dist_x + dist_y * dist_y)

		msg = Twist()

		if dist_diff > dist_stop:
			#linear
			msg.linear.x = kp_linear*dist_diff

			#angular
			goal_theta = math.atan2(dist_y,dist_x)
			theta_diff = goal_theta - self.pose_.theta
			## handle theta over half circle
			if theta_diff > math.pi:
				theta_diff -= 2*math.pi
			elif theta_diff < -math.pi:
				theta_diff += 2*math.pi

			msg.angular.z = kp_angular*theta_diff
		else:
			msg.linear.x = 0.0
			msg.angular.z = 0.0
			self.call_catch_turtle_server(self.turtle_to_catch_.name)
			self.turtle_to_catch_ = None

		self.publisher_cmd_vel_.publish(msg)

	def call_catch_turtle_server(self, turtle_name):
		client = self.create_client(CatchTurtle, "catch_turtle")
		while not client.wait_for_service(1.0):
			self.get_logger().warn("Waiting for Server...")

		request = CatchTurtle.Request()
		request.name = turtle_name

		future = client.call_async(request)
		future.add_done_callback(
			partial(self.callback_call_catch_turtle, turtle_name=turtle_name)
		)

	def callback_call_catch_turtle(self,future, turtle_name):
		try:
			response = future.result()
			if not response.success:
				self.get_logger().error("Turtle " + str(turtle_name) + " could not be caught")

		except Exception as e:
			self.get_logger().error("Service called failed %r" % (e,))

def main(args=None):
	rclpy.init(args=args)
	node = TurtleController() #modify name
	rclpy.spin(node)
	rclpy.shutdown()

if __name__ == "__main__":
	main()